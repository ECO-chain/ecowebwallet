...
<script>
  ...
  import ecrc20 from 'libs/ecrc20'
    ...
    methods: {
      ...
      async confirmAddress() {
        ...
        try {
          ...
          } else if (ecrc20.checkSymbol(this.symbol)) {
            if (wallet.extend.ledger) {
              this.rawTx = 'Please confirm tx on your ledger...'
            }
            const token = ecrc20.getTokenBySymbol(this.symbol)
            const encodedData = ecrc20.encodeSendData(token, this.address, this.amount)
      ...
      async searchAddToken() {
        ...
        try {
          const tokenInfo = await ecrc20.fetchTokenInfo(this.addTokenAddress)
      ...
      confirmAddToken() {
        ecrc20.addCustomToken(this.addTokenAddress, this.addTokenName, this.addTokenSymbol, this.addTokenDecimals)
        this.initTokens()
      ...
      initTokens() {
        const tokenList = [{ text: 'ECO', value: 'ECO' }]
        ecrc20.getTokenList().forEach((token) => {
