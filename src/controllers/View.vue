  ...
  <v-card>
    ...
    <v-card-text>
      <v-layout v-for="(item, i) in infoLabel" :key="i">
        ...
        <v-flex xs2>
          <v-btn small class="mt-3" color="purple darken-3" v-clipboard:copy="info[item.name]" v-clipboard:success="onCopySucc" v-clipboard:error="onCopyError" v-if="item.copy">
            {{ $t('common.copy') }}
          </v-btn>
        </v-flex>
      </v-layout>
      <v-layout v-if="privKey !== null">
        ...
        <v-flex xs2>
          <v-btn small class="mt-3" color="purple darken-3" v-show="showPriv" v-clipboard:copy="privKey" v-clipboard:success="onCopySucc" v-clipboard:error="onCopyError">
            {{ $t('common.copy') }}
          </v-btn>
        </v-flex>
      </v-layout>
      <v-layout v-if="wallet.info.ecrc20.length > 0">
        <v-flex xs3>
          <v-subheader>
            ECRC20
          </v-subheader>
        ...
        <v-flex xs7>
          <v-card flat tile :color="'grey darken-'+(i%2+1)" style="border-bottom: 2px solid #000;padding: 15px 10px;" v-for="(token, i) in wallet.info.ecrc20" :key="i">
            <v-layout>
